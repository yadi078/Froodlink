...
;(($) => {
  ...
  function displayMenu(comidas, user) {
    ...
    const imageLinks = {
      "Tacos al Pastor": "img/tacos-al-pastor-small-10.jpg",
      "Pozole Rojo": "img/Pozole_rojo-5_480x480.webp",
      "Quesadillas": "img/quesadillas.jpeg",
      "Enchiladas Verdes": "img/enchiladas.jpeg"
    }
    ...
    comidas.forEach((comida) => {
      ...
      const imgSrc = imageLinks[comida.nombre_comida] || "img/default.jpg"
        ...
        <div class="col-lg-6 col-md-12 mb-4">
          <div class="card h-100">
            <div class="row no-gutters">
              <div class="col-md-4">
                <img src="${imgSrc}" class="card-img" alt="${comida.nombre_comida}" style="height: 100%; object-fit: cover;">
              </div>
              <div class="col-md-8">
                <div class="card-body">
                  ...
                  ${
                    !isAuthenticated
                      ? `<div class="alert alert-warning"><small><i class="fas fa-lock"></i> Debes iniciar sesión para ordenar</small></div>`
                      : canOrder
                        ...
                        : `<button class="btn btn-sm btn-secondary" disabled><i class="fas fa-times"></i> No Disponible</button>`
                  }
                  ...
                  <div class="mt-3 comments-section">
                    ...
                    ${
                      isAuthenticated
                        ...
                        : `<div class="alert alert-info mt-2"><small><i class="fas fa-lock"></i> Debes iniciar sesión para comentar</small></div>`
                    }
                  </div>
                </div>
              </div>
    ...
    $(".comment-form").submit(function (e) {
      ...
      if (comment) submitComment(foodId, comment, form)
    })
  }

  // Funciones completas que faltaban
  function loadComments(foodId) {
    $.ajax({
      ...
      success: (response) => {
        if (response.success) displayComments(foodId, response.comentarios)
      },
      error: () => {
  ...
  function displayComments(foodId, comentarios) {
    const container = $(`#comments-${foodId}`)
    if (comentarios.length === 0) {
      container.html('<p class="text-muted"><small>No hay comentarios aún. Sé el primero en comentar.</small></p>')
      return
    }
    let html = ""
  ...
  function calculateTotal() {
    ...
    $("#orderTotal").val("$" + (quantity * price).toFixed(2))
  }

  function createOrder() {
    const user = window.FoodLinkAuth.getUser()
    if (!user) { window.FoodLinkAuth.showAlert("danger", "Debes iniciar sesión para ordenar"); return }
    ...
    $.ajax({
      ...
      method: "POST",
      data: { id_usuario: user.id_usuario, id_comida: foodId, cantidad: quantity },
      success: (res) => {
        if (res.success) {
          $("#orderModal").modal("hide")
          window.FoodLinkAuth.showAlert("success", "Pedido realizado exitosamente")
          setTimeout(() => loadMenu(), 1000)
        } else window.FoodLinkAuth.showAlert("danger", res.message || "Error al crear el pedido")
      },
      error: (xhr) => {
        const res = xhr.responseJSON || {}
        window.FoodLinkAuth.showAlert("danger", res.message || "Error al conectar con el servidor")
      },
    })
  }
  ...
  function submitComment(foodId, comment, form) {
    const user = window.FoodLinkAuth.getUser()
    if (!user) { window.FoodLinkAuth.showAlert("danger", "Debes iniciar sesión para comentar"); return }

    $.ajax({
      url: API_URL + "/add_comentario.php",
      method: "POST",
      data: { id_usuario: user.id_usuario, id_comida: foodId, comentario: comment },
      success: (res) => {
        if (res.success) {
        ...
        } else window.FoodLinkAuth.showAlert("danger", res.message || "Error al agregar comentario")
      },
      error: (xhr) => {
        const res = xhr.responseJSON || {}
        window.FoodLinkAuth.showAlert("danger", res.message || "Error al conectar con el servidor")
